require([
    ...
    var greeks = ['alpha', 'beta', 'gamma', 'delta', 'epsilon', 'zeta', 'eta', 'theta', 'iota', 'kappa'];
    Handlebars.registerHelper("foreach",function(arr,options) {
        ...
        return arr.map(function(item,index) {
            ...
            item.$class_name = greeks[currentClassIndex];
            item.$first = index === 0;
    ...
    require(['stickUp'], function(stickUp) {
      ...
      $('#second-navbar-container').stickUp({
        topMargin: 20
      });
      ...
      $('section, section > div').each(function(){
        sections[this.id] = $(this).offset().top + 50;
      });
    ...
    if ($.urlParam('compare')) {
        ...
        if (window.location.hash) {
            $('html,body').animate({ scrollTop: parseInt($(window.location.hash).offset().top) - 18 }, 0);
        }
    }
